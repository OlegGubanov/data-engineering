...
def write_memory_stats(filename, df):
    file_size = os.path.getsize('[4]vacancies.csv.gz')
...
df = pd.read_csv('[4]vacancies.csv.gz', compression='gzip')
...
columns_dtypes = {
    'schedule_name': pd.StringDtype,
    'accept_handicapped': pd.CategoricalDtype,
    'accept_kids': pd.CategoricalDtype,
    'experience_name': pd.StringDtype,
    'billing_type_name': pd.StringDtype,
    'premium': pd.CategoricalDtype,
    'type_name': pd.StringDtype,
    'salary_from': pd.StringDtype,
    'salary_to': pd.StringDtype,
    'salary_gross': pd.StringDtype,
    'employment_name': pd.StringDtype
}

write_header = True
for chunk in pd.read_csv('[4]vacancies.csv.gz',
                         compression='gzip',
