...
def write_memory_stats(filename, df):
    file_size = os.path.getsize('[5]asteroid.zip')
...
columns_dtypes = {
    ...
    'epoch_cal': pd.StringDtype,
    'tp_cal': pd.StringDtype,
    'per': pd.StringDtype,
    'class': pd.CategoricalDtype,
    'albedo': pd.StringDtype,
    'sigma_q': pd.StringDtype,
    'sigma_e': pd.StringDtype,
    'sigma_a': pd.CategoricalDtype
}

write_header = True
for chunk in pd.read_csv('[5]asteroid.zip',
