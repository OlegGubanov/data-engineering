...
columns_dtypes = {
    'YEAR': pd.CategoricalDtype,
    'AIRLINE': pd.CategoricalDtype,
    'ORIGIN_AIRPORT': pd.CategoricalDtype,
    'DESTINATION_AIRPORT': pd.CategoricalDtype,
    'SCHEDULED_DEPARTURE': pd.StringDtype,
    'DEPARTURE_DELAY': pd.StringDtype,
    'SCHEDULED_ARRIVAL': pd.StringDtype,
    'ARRIVAL_TIME': pd.StringDtype,
    'ARRIVAL_DELAY': pd.StringDtype,
    'DISTANCE': pd.StringDtype,
    'CANCELLED': pd.CategoricalDtype
}

write_header = True
for chunk in pd.read_csv('[3]flights.csv',
