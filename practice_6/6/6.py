...
def write_memory_stats(filename, df):
    file_size = os.path.getsize('vehicles.csv.zip')
...
columns_dtypes = {
    ...
    'region_url': pd.CategoricalDtype,
    'price': pd.StringDtype,
    'year': pd.CategoricalDtype,
    'manufacturer': pd.CategoricalDtype,
    'model': pd.CategoricalDtype,
    'condition': pd.CategoricalDtype,
    'cylinders': pd.CategoricalDtype
}

write_header = True
for chunk in pd.read_csv('vehicles.csv.zip',
